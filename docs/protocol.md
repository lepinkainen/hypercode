...
Prompts are `{"type":"user","message":{"role":"user","content":[{"type":"text","text":…}]}}` lines on stdin. Image attachments add `{"type":"image","source":{"type":"base64","media_type":"image/png","data":"…"}}` blocks; image-only turns omit the text block. The CLI never acknowledges a prompt; the `result` frame ends the turn. Text arrives as `stream_event` frames wrapping Messages API stream events: `text_delta` becomes `text_delta`, `content_block_stop` becomes `text_done` with the accumulated block. The item id is `<message id>-<block index>`. Tool calls become `tool_started` from the `tool_use` block of the `assistant` frame (complete input) and `tool_done` from the matching `tool_result` block of a `user` frame; `is_error` marks the item failed. Frames with a non-null `parent_tool_use_id` belong to subagents and are dropped. Thinking deltas are not rendered.
...
Verified on 2026-09-10 with codex-cli 0.153.4 and Claude Code 2.1.266. Both spike commands accept `-image /absolute/path/image.png`; pass `-prompt ''` to verify an image-only turn. Use `-record` for dedicated protocol captures.

Codex adds `{"type":"localImage","path":"/absolute/path/image.png"}` to `turn/start.input`. Claude uses the base64 block described above. Both accepted an image-only turn and recognized a red test image. Cold resume retained its color in both agents. Claude must be able to write its native session history for cold resume to work.

Documents are local file references in text input, not native document blocks. Verification used a text file and a PDF outside the chat working directory. Claude's Read tool read the PDF directly, while Codex used the installed `pdftotext`. Claude requested Read approvals outside the working directory in workspace-write mode. Native settings and sandbox policies determine whether a read requires approval or is denied; Hypercode keeps those policies intact.

The offline peers validate image paths or base64 blocks and report the received image count and byte count. Adapter tests cover image-only, multiple-image, and mixed document/image requests without model calls.
